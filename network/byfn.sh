...
function networkUp() {
    ...
    if [ "${NO_CHAINCODE}" != "true" ]; then
        ...
        pushd ../chaincode/chaincode_token/go
    ...
    docker exec cli.demo scripts/script.sh $CHANNEL_NAME $CLI_DELAY $CC_SRC_LANGUAGE $CLI_TIMEOUT $VERBOSE $NO_CHAINCODE
    if [ $? -ne 0 ]; then
...
function upgradeNetwork() {
    if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
        ...
        docker-compose $COMPOSE_FILES stop cli.demo
        docker-compose $COMPOSE_FILES up -d --no-deps cli.demo
        ...
        docker exec cli.demo scripts/upgrade_to_v14.sh $CHANNEL_NAME $CLI_DELAY $CC_SRC_LANGUAGE $CLI_TIMEOUT $VERBOSE
        if [ $? -ne 0 ]; then
            echo "ERROR !!!! Test failed"
            exit 1
        fi
...
function generateChannelArtifacts() {
    ...
    configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
        ./channel-artifacts/Org2MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org2
    res=$?
    set +x
    if [ $res -ne 0 ]; then
...
CC_SRC_LANGUAGE=go
# default image tag
CFG_IMAGE_TAG=$(cat .env | grep 'IMAGE_TAG')
IMAGETAG=${CFG_IMAGE_TAG#*=}
